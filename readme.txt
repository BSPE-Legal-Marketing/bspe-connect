...
= 3.8.0 = New Translate tab: Claude-powered page translation for WPML sites. Paste a page URL, pick the language (Spanish for now), press Translate. The plugin extracts everything WPML would translate (title, slug, excerpt, post content, Elementor widget text inside _elementor_data including repeaters, SEOPress title/description and social fields, and custom fields WPML marks as translate), sends it to the Claude API in chunks (~12k characters per request, long HTML split at block boundaries so big Elementor pages never time out), then creates the WPML-linked translation post: same status as the original, all other postmeta copied, Elementor CSS regenerated, terms mapped to their WPML translations. Existing translations can be overwritten in place after an explicit tick. Nothing is written until every chunk came back; failures show the API message and offer Retry on the same chunk. Settings: API key (stored, never re-displayed), model (Opus 5 default, Sonnet 5), default language, firm name (never translated). Available only with an active license, WPML running and a saved key; every completed job is logged with token usage. Cost is shown for admins: an estimate before translating, the exact dollar cost after each page (tokens x list price), and a Translation costs card with the site's all-time total, pages translated, average per page, and a recent-jobs table.
= 3.7.3 = The form modal title ("Send us an email" / "Send us a text") is now a <p>, not an <h2>, so it stays out of the page's heading outline for SEO. Styling is unchanged: the rule is class-based with a #bspe-connect prefix, so it renders identically and theme paragraph styles cannot touch it (and it cannot affect other paragraphs). aria-labelledby still points at the same element, so the dialog keeps its accessible name. This was the only H tag in the plugin's frontend output.
