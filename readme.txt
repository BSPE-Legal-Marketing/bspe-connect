...
= 3.8.0 = Fleet translation support (no UI on the site). New signed REST endpoints under bspe-connect/v1/translate/* let the BSPE Connect Manager translate a page into Spanish from the fleet dashboard: extract returns everything WPML would translate (title, slug, excerpt, content, Elementor widget text inside _elementor_data including repeaters, SEOPress title/description/social fields, WPML-marked custom fields; long HTML pre-split at block boundaries), apply creates or updates the WPML-linked translation with the same status as the original, copies all other postmeta, regenerates Elementor CSS and maps terms to their WPML translations. Requests must carry a short-lived RS256 token signed by the Manager for this exact domain and license key (same key pair as license checks); the endpoints do nothing without an active license and WPML. License check/activate now also reports the site URL so the Manager knows where to call. Nothing changes in wp-admin.
