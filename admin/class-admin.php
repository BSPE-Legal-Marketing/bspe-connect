...
final class Admin {
	...
	public static function init(): void {
		...
		// Admin-wide nudge when WPML is running but the schema stripper
		// is off (translated pages are carrying default-language JSON-LD),
		// plus a warning on the edit screen of translated pages whose
		// schema will be stripped.
		add_action( 'admin_notices', [ self::class, 'render_wpml_schema_notice' ] );
		add_action( 'admin_notices', [ self::class, 'render_post_schema_stripped_notice' ] );
		add_action( 'admin_footer', [ self::class, 'print_seopress_schema_warning' ] );
	...
	/**
	 * Admin-wide warning when WPML is active but the schema stripper is
	 * off: translated pages are serving the default language's JSON-LD,
	 * which is wrong structured data for those URLs. Mirrors the
	 * Skip-language notice pattern; disappears once the utility is on.
	 */
	public static function render_wpml_schema_notice(): void {
		if ( ! current_user_can( self::CAPABILITY ) ) {
			return;
		}
		if ( ! \BSPE\Connect\WPML_Status::wpml_active() ) {
		...
		if ( (bool) \BSPE\Connect\Settings::get( 'utilities.wpml_strip_schema', false ) ) {
			return;
		}
		?>
		<div class="notice notice-warning">
			<p>
				<strong><?php esc_html_e( 'WPML detected:', 'bspe-connect' ); ?></strong>
				<?php esc_html_e( 'translated pages are outputting the default language\'s structured data (schema), which is wrong for those URLs. Turn on "Strip schema on translated pages" under General, Site utilities.', 'bspe-connect' ); ?>
	...
	/**
	 * Warning at the top of the edit screen of a translated post whose
	 * schema output will be stripped on the frontend, so nobody wastes
	 * time authoring a custom schema that never renders. Names the post
	 * ID to paste into the allow list.
	 */
	public static function render_post_schema_stripped_notice(): void {
		$post_id = self::current_edit_post_id();
		if ( ! $post_id || ! current_user_can( self::CAPABILITY ) ) {
			return;
		}
		if ( ! \BSPE\Connect\WPML_Schema_Stripper::post_gets_stripped( $post_id ) ) {
			return;
		}
		?>
		<div class="notice notice-warning">
			<p>
				<strong><?php esc_html_e( 'Schema is stripped on this page:', 'bspe-connect' ); ?></strong>
				<?php
				printf(
					/* translators: %d: post ID */
					esc_html__( 'this is a translated page, so BSPE Connect removes all structured data (JSON-LD) from it on the frontend. If this page has its own correct schema, allow it by adding ID %d to "Pages allowed to keep schema" under General, Site utilities.', 'bspe-connect' ),
					(int) $post_id
				);
				?>
				<a href="<?php echo esc_url( self::tab_url( 'general' ) ); ?>"><?php esc_html_e( 'Open Site utilities', 'bspe-connect' ); ?> &rarr;</a>
	...
	/**
	 * Best-effort companion to the edit-screen notice: injects the same
	 * warning INSIDE the SEOPress metabox's Schemas panel (where custom
	 * schemas are authored), since that panel opens as an overlay far
	 * from the top-of-page notice. Watches the DOM for the panel
	 * (SEOPress renders it lazily) and prepends a highlighted note to
	 * the smallest SEOPress container holding the schema UI.
	 */
	public static function print_seopress_schema_warning(): void {
		$post_id = self::current_edit_post_id();
		if ( ! $post_id || ! current_user_can( self::CAPABILITY ) ) {
			return;
		}
		...
		$msg = sprintf(
			/* translators: %d: post ID */
			__( 'BSPE Connect: this is a translated page, so any schema saved here is stripped from the live page. To keep it, add page ID %d to "Pages allowed to keep schema" in BSPE Connect, General, Site utilities.', 'bspe-connect' ),
			(int) $post_id
		);
		?>
		<script>
		(function () {
			'use strict';
			var TEXT = <?php echo wp_json_encode( $msg ); ?>;
			var MARKERS = ['Select your data type', 'Add a schema', 'Validate my schema'];
			function containsMarker(el) {
			...
			function inject() {
				if (window.__bspeSchemaNoteDone) { return true; }
				var nodes = document.querySelectorAll('[class*="seopress"], [id*="seopress"]');
				var best = null, bestCount = Infinity;
				for (var i = 0; i < nodes.length; i++) {
					if (!containsMarker(nodes[i])) { continue; }
					var c = nodes[i].getElementsByTagName('*').length;
					if (c < bestCount) { best = nodes[i]; bestCount = c; }
				}
				if (!best) { return false; }
				var div = document.createElement('div');
				div.setAttribute('style', 'margin:10px 0;padding:10px 14px;border-left:4px solid #dba617;background:#fcf9e8;font-size:13px;line-height:1.5;color:#1d2327;');
				div.textContent = TEXT;
				best.insertBefore(div, best.firstChild);
				window.__bspeSchemaNoteDone = true;
				return true;
			}
			if (inject()) { return; }
			try {
			...
			} catch (e) { /* no observer support: the top notice still covers it */ }
		})();
		</script>
		<?php
	}
	...
	/**
	 * ID of the post being edited when the current admin screen is the
	 * classic/block post editor, 0 otherwise.
	 */
	private static function current_edit_post_id(): int {
		...
		return isset( $_GET['post'] ) ? (int) $_GET['post'] : 0; // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- read-only screen detection
	}
