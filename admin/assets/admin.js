...
(function () {
	...
	/* ---------------------------------------------------------------- */
	/*  Translate tab — chunked Claude translation                      */
	/* ---------------------------------------------------------------- */
	function initTranslate() {
		var root = document.querySelector('[data-bspe-translate]');
		if (!root || !window.bspeTranslate || !window.fetch) { return; }

		var cfg  = window.bspeTranslate;
		var i18n = cfg.i18n || {};
		var q    = function (sel) { return root.querySelector(sel); };

		var urlInput   = q('[data-bspe-translate-url]');
		var lookupBtn  = q('[data-bspe-translate-lookup]');
		var errorEl    = q('[data-bspe-translate-error]');
		var panels     = root.querySelectorAll('[data-bspe-translate-panel]');
		var titleEl    = q('[data-bspe-translate-title]');
		var metaEl     = q('[data-bspe-translate-meta]');
		var editLink   = q('[data-bspe-translate-edit]');
		var existingEl = q('[data-bspe-translate-existing]');
		var langwarnEl = q('[data-bspe-translate-langwarn]');
		var targetSel  = q('[data-bspe-translate-target]');
		var owWrap     = q('[data-bspe-translate-overwrite-wrap]');
		var owBox      = q('[data-bspe-translate-overwrite]');
		var startBtn   = q('[data-bspe-translate-start]');
		var cancelBtn  = q('[data-bspe-translate-cancel]');
		var progress   = q('[data-bspe-translate-progress]');
		var bar        = q('[data-bspe-translate-bar]');
		var statusEl   = q('[data-bspe-translate-status]');
		var result     = q('[data-bspe-translate-result]');
		var resultText = q('[data-bspe-translate-result-text]');
		...
		function fmt(str) {
			...
			return String(str).replace(/%(\d+\$)?[sd]/g, function () { return args[i++]; });
		}
		function num(n) { return Number(n || 0).toLocaleString(); }
		function showError(msg, retryFn) {
			errorEl.textContent = msg || '';
			errorEl.hidden = !msg;
			var old = errorEl.querySelector('button');
			if (old) { old.parentNode.removeChild(old); }
			if (msg && retryFn) {
				var b = document.createElement('button');
				b.type = 'button';
				b.className = 'bspe-button bspe-button--secondary bspe-translate__retry';
				b.textContent = i18n.retry || 'Retry';
				b.addEventListener('click', function () { showError(''); retryFn(); });
		...
		function post(action, data) {
			var fd = new FormData();
			fd.append('action', 'bspe_connect_translate_' + action);
			...
			return fetch(cfg.ajaxUrl, { method: 'POST', body: fd, credentials: 'same-origin' })
				...
				.then(function (json) {
					if (!json || json.success === false) {
						var e = new Error((json && json.data && json.data.message) || (i18n.network || 'Network error.'));
						e.retry = !!(json && json.data && json.data.retry);
						throw e;
					}
					return json.data || {};
				}, function () { throw new Error(i18n.network || 'Network error.'); });
		}
		function setBusy(b) {
			running = b;
			lookupBtn.disabled = b;
			startBtn.disabled = b;
			targetSel.disabled = b;
			urlInput.disabled = b;
			cancelBtn.hidden = !b;
		}
		...
		function refreshExisting() {
			...
			var tr = page.translations && page.translations[target];
			var label = targetSel.options[targetSel.selectedIndex].text;
			if (tr && tr.id > 0) {
				existingEl.textContent = fmt(i18n.existing || 'A %s translation already exists (status: %s).', label, tr.status);
				if (tr.edit_url) {
					existingEl.appendChild(document.createTextNode(' '));
					var a = document.createElement('a');
					a.href = tr.edit_url; a.target = '_blank'; a.rel = 'noopener'; a.textContent = 'Edit';
					existingEl.appendChild(a);
				}
			...
			langwarnEl.hidden = page.language === page.default_lang;
			if (!langwarnEl.hidden) {
				langwarnEl.textContent = fmt(i18n.langwarn || '', page.language, page.default_lang);
			}
		}
		...
		function lookup() {
			...
			statusEl.textContent = i18n.looking || '';
			post('lookup', { url: url }).then(function (data) {
				page = data;
		...
		function start() {
			...
			cancelled = false;
			setBusy(true);
			setProgress(0, 1, i18n.preparing || '');
			post('start', { post_id: page.id, target: targetSel.value, overwrite: owBox.checked ? 1 : 0 }).then(function (data) {
				job = { id: data.job, total: data.batches, chars: data.chars };
				return step();
			}).catch(function (e) {
				setBusy(false);
				progress.hidden = true;
		...
		function step() {
			...
			return post('step', { job: job.id }).then(function (p) {
				job.total = p.total;
				setProgress(p.chars_done, p.chars, fmt(i18n.batch || '', Math.min(p.next + 1, p.total), p.total, num(p.chars_done), num(p.chars)));
				if (p.finished) { return apply(); }
				return step();
			}).catch(function (e) {
				setBusy(false);
				showError(e.message, e.retry ? function () { setBusy(true); step(); } : null);
			});
		}

		function apply() {
			setProgress(1, 1, i18n.applying || '');
			return post('apply', { job: job.id }).then(function (data) {
				setBusy(false);
				progress.hidden = true;
				...
				post('lookup', { url: urlInput.value.trim() }).then(function (d) { page = d; refreshExisting(); }).catch(function () {});
			}).catch(function (e) {
				setBusy(false);
				showError(e.message, function () { setBusy(true); apply(); });
			});
		}

		function cancel() {
			cancelled = true;
			if (job) { post('cancel', { job: job.id }).catch(function () {}); }
			job = null;
			setBusy(false);
			progress.hidden = true;
		}
		...
		urlInput.addEventListener('keydown', function (ev) { if (ev.key === 'Enter') { ev.preventDefault(); lookup(); } });
		targetSel.addEventListener('change', refreshExisting);
